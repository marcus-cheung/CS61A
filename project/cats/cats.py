...
def about(topic):
    ...
    def helper(paragraph):
        p = remove_punctuation(lower(paragraph))
        split = p.split()
        for x in topic:
            if split.count(x)>0:
                return True
...
def accuracy(typed, reference):
    ...
    if typed=='' and reference=='':
        ...
        return 0.0
    count = 0
    splittype = typed.split()
    splitref = reference.split()
    small = min(len(splitref),len(splittype))
    for i in range(small):
        if splittype[i]==splitref[i]:
            count+=1
    return count/len(splittype) *100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for x in valid_words:
        if typed_word==x:
            return typed_word
        dct[f'{x}'] = diff_function(typed_word,x,limit)
    if min(dct.values()) <= limit:
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        ...
        if typed[i]==prompt[i]:
            ...
            break
    info = {'id': user_id, 'progress': count/len(prompt)}
    send (info)
...
def time_per_word(times_per_player, words):
    ...
    for x in times_per_player:
        ...
        tlist = []
        while index<len(x)-1:
            tlist.append(x[index+1]-x[index])
            index+=1
        times.append(tlist)
...
def fastest_words(game):
    ...
    playerlist = [[] for i in player_indices]
    playerwordlist = [[]for i in player_indices] 
    for index in word_indices:
        lowest = times[0][index]
        fastestplayerindex = 0
        for player in player_indices:
            if lowest>times[player][index]:
                lowest = times[player][index]
                fastestplayerindex = player
        playerlist[fastestplayerindex].append(index)
    
    
    for playerindex in range(len(playerlist)):
        for word in playerlist[playerindex]:
            playerwordlist[playerindex].append(word_at(game,word))
