...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    scoresq = score * score
    digits = len(str(scoresq))
    smallest = scoresq%10
    i=0
    while i < digits:
        if smallest>scoresq%10:
            smallest = scoresq%10
        scoresq //= 10
        i += 1
    return smallest+3
...
def more_boar(player_score, opponent_score):
    ...
    LeftO = opponent_score
    iO = 1
    iO2 = 1
    lenO = len(str(opponent_score))
    SLeftO = opponent_score
    ...
    return LeftP < LeftO and SLeftP%10 < SLeftO%10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    goalcheck = lambda x,y: x>=goal or y>=goal
    

    def turn (strat,player,opponent,s,who):
        if goalcheck(player,opponent):
            return player,opponent,s,next_player(who)
        
        player += take_turn(strat(player,opponent), opponent, dice, goal)
        if who==0:
            s = s(player,opponent)
        else:
            s = s(opponent,player)
        ...
        while more_boar(player,opponent):
            if goalcheck(player,opponent):
                return player,opponent,s,next_player(who)
            player += take_turn(strat(player,opponent), opponent, dice, goal)
            if who==0:
    ...
    while goalcheck(score0,score1) != True:
            score0,score1,say,who = turn(strategy0,score0,score1,say,who)
            if goalcheck(score0,score1) == True:
                return score0,score1
            score1,score0,say,who = turn(strategy1,score1,score0,say,who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(p0,p1):
        r = running_high
        if who==0:
            difference = p0 - last_score
        elif who==1:
            difference = p1 - last_score
        if difference > r:
            print ('Player', who, 'has reached a new maximum point gain.', difference,'point(s)!')
            r = difference
        if who==0:
            return announce_highest(who,p0,r)
        else:
            return announce_highest(who,p1,r)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if more_boar(score+piggy_points(opponent_score), opponent_score) or piggy_points(opponent_score)>=cutoff else num_rolls # Replace this statement
    # END PROBLEM 11
