    ...
    def helper(lst,sum):
        if len(s) == 0:
            if abs(sum)<=d:
                ...
                return 0
        return helper(s[1:], sum-s[0]) + helper(s[1:], sum+s[0])
    return helper(s,0)
...
class Account:
    ...
    def deposit(self, amount):
        ...
        """
        self.balance+=amount
        self.transactions.append(tuple('deposit', amount))
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        """
        self.balance-=amount
        self.transactions.append(tuple('withdraw', amount))
        return self.balance
    ...
    def __repr__(self):
        deposits = len([x for x in self.transactions if x[0]=='deposit'])
        withdraws = len([x for x in self.transactions if x[0]=='withdraw'])
        return f'Accountholder: {self.account_holder}, Deposits: {deposits}, Withdraws: {withdraws}'
